...
def ai_generate_text(prompt: str, api_key: str, system: str = None, model: str = "gemini-2.0-flash") -> str:
    """
    تماس با Google Gemini (رایگان). کلید رو از aistudio.google.com بگیر.
    """
    if not api_key or api_key == "PUT_YOUR_GEMINI_KEY_HERE":
        return "برای این قابلیت باید کلید رایگان Gemini رو تنظیم کنی (GEMINI_API_KEY)."

    url = f"https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent"
    contents = []
    if system:
        contents.append({"role": "user", "parts": [{"text": system}]})
        contents.append({"role": "model", "parts": [{"text": "باشه، فهمیدم."}]})
