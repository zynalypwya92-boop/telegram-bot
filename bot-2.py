...
async def handle_wizard_input(update: Update, context: ContextTypes.DEFAULT_TYPE, awaiting: str, text: str):
    ...
    elif awaiting == "add_reply_trigger":
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("هوش مصنوعی"):
        ...
        if len(rest) > 1 and rest[1].strip():
            ...
            answer = tools.ai_generate_text(question, GEMINI_API_KEY, system=system_prompt)
    ...
    if is_greeting(text):
        ...
        ai_reply = tools.ai_generate_text(prompt, GEMINI_API_KEY)
        if nickname:
    ...
    if text == "جوک":
        ...
        joke = tools.ai_generate_text(prompt, GEMINI_API_KEY)
    ...
    if text == "فال":
        ...
        await update.message.reply_text(tools.ai_generate_text(prompt, GEMINI_API_KEY))
        return
...
async def summary_job(context: ContextTypes.DEFAULT_TYPE):
    for chat_id in database.get_all_chats_with_recent_messages():
        ...
        summary = tools.ai_generate_text(prompt, GEMINI_API_KEY)
        try:
